...
public class PlayerControl extends ListenerAdapter {
  ...
  */
    ...
    playerManager.loadItemOrdered(
        ...
        new AudioLoadResultHandler() {
          ...
          public void playlistLoaded(AudioPlaylist playlist) {
            ...
            // channel.sendMessage("Adding to queue " + firstTrack.getInfo().title + " (first track
            // of playlist " + playlist.getName() + ")").queue();
  ...
  private void repeatTrack(final TextChannel channel, MessageReceivedEvent event) {
    ...
    // event.getChannel().sendMessage("Player was set to: **" + (scheduler.isRepeating() ? "repeat"
    // : "not repeat") + "**").queue();
