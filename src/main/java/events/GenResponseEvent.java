...
class SanitizeAIOutput {
  ...
  private static final List<String> REPLACEMENTS =
      Arrays.asList("*nice try*", "*nope*", "*try something else*", "*goku*");

  private static final Random RANDOM = new Random();

  /**
   * Checks a string for specific sequences and replaces them with a random string from the
   * hard-coded REPLACEMENTS list.
   */
  public static String sanitizeOutput(MessageReceivedEvent event, String message) {
    ...
    while (matcher.find()) {
      ...
      event
          .getJDA()
          .getUserById("232295708974120960")
          .openPrivateChannel()
          .queue(
              (channel) -> {
...
public class GenResponseEvent extends ListenerAdapter {
  ...
  public static boolean isZabaOnline(MessageReceivedEvent event) {
    ...
    if (aiMODE.equals("DEBUG")) {
      if (!event.getGuild().getId().contains("944254135476305980")) return false;
    } else if (aiMODE.equals("LIMITED")) {
      if (!limitedChannels.contains(event.getChannel().getId())) return false;
    }
  ...
  private void callHelp(MessageReceivedEvent event) {
    ...
    if (help.length == 1) {
      ...
      eb.setThumbnail(
          "https://images-ext-1.discordapp.net/external/HwnSW1Qv1B0_ZkomUohZ7P-TYmFsX775K0H4CdQRbAw/https/e0.pxfuel.com/wallpapers/940/704/desktop-wallpaper-glass-animals-zaba-artwork-by-micah-lidberg-glass-animals-pool-thumbnail.jpg");
    ...
    */
    if (help[1].equalsIgnoreCase("socialcredit")) {
      event
          .getChannel()
          .sendMessage(
              "多黨制\t\t**Social Credit - How to perform your 中国共产党 Duty**"
                  ...
                  + "\n*李洪志*\t   ● Use '!show all' or '!show mine' to view social credit scores")
          .queue();
      return;
    }
    if (help[1].equalsIgnoreCase("contra")) {
      ...
      command.add(
          "This function allows users to claim an item on a list, providing a comprehensive tracker for who brings or does what for an event.\n\n");
      command.add(
          "Any user can add, remove, or reset the list. You can claim as many different items on the list as you want.\n");
      call.add("!contra\n\n");
      calldesc.add("Summons tracker and allows users to add their name\n");
      call.add("!contra add XYZ\n\n\n");
      calldesc.add(
          "Replace XYZ with the name of the item you'd like to add to the list (spaces allowed)\n");
      call.add("!contra del XYZ\n\n\n");
      calldesc.add(
          "Replace XYZ with the name of the item you'd like remove from the list (case sensitive)\n");
      call.add("!contra clear\n\n");
      calldesc.add("Resets the tracker, deleting all users and entries\n");

      for (String s : command) commands.append(s);
      for (String s : call) calls.append(s);
      for (String s : calldesc) calldescs.append(s);
    ...
    if (help[1].equalsIgnoreCase("swearjar")) {
      ...
      command.add(
          "Zaba tracks everyone's use of specific words. The master word list is unknown to everyone but Zaba. \n\n");
      call.add("!swearjar\n");
  ...
  private String callGatekeeperAI(String prompt) {
    try {
      ...
      payload.addProperty(
          "model", "gatekeeper"); // Ensure a model named "gatekeeper" exists in OpenWebUI
  ...
  public void onMessageReceived(MessageReceivedEvent event) {
    ...
    if (unapprovedChannels.contains(event.getChannel().getId())) return;

    // GenEm or General Emote will react with an emote whenever a trigger word / phrase is detected
    // in a message
    ...
    if (isZabaOnline(event)) {
      ...
      System.out.println(
          "Gatekeeper: " + judgment + " from user: " + event.getMessage().getAuthor().getName());
      switch (judgment) {
        case "ZABA":
          callZabaAI(event);
          break;
    ...
    for (String str : scanGen) {
      if (msg.contains(str)) {
        System.out.println(
            " #" + event.getChannel().getName() + " @" + event.getMessage().getAuthor().getName());
        event.getChannel().sendMessage(gen.get(str).toString()).queue();
      }
    }

    // GenEx or General Exact will respond only when the message in question matches exactly with
    // the trigger list
    assert genEx != null;
    ...
    // GenCom or General Complex will trigger from a list of words and pull a random response from
    // another list
    assert genCom != null;
          ...
          if (Math.random() > .65) {
            ...
            MessageCreateData data =
                new MessageCreateBuilder()
                    .setFiles(
                        FileUpload.fromData(new File("videos/" + genScan.get(key) + "/" + meme)))
                    .build();
            event.getChannel().sendMessage(data).queue();
          }
        } else {
    ...
    // GenProb or General Probability when triggered will pull from a list of responses accompanied
    // by a chance of the response being given
    assert genProb != null;
    ...
    // GenZaba or General Zaba is like General but specifically when talking to Zaba, keeps things
    // organized
    assert genZaba != null;
