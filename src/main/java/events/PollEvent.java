...
public class PollEvent extends ListenerAdapter {
  ...
  public void onMessageReceived(MessageReceivedEvent event) {
    ...
    if (cmd.equalsIgnoreCase("!poll")) {
      ...
      MessageCreateData data = new MessageCreateBuilder().addEmbeds(builder.build()).build();
            ...
            .thenAccept(
                message -> {
                  for (int i = 1; i < finalArgs.length; i++) {
                    message
                        .addReaction(Emoji.fromUnicode(Arrays.asList(reactions).get(i - 1)))
                        .queue();
                  }
                });
  ...
  public void onMessageReactionAdd(MessageReactionAddEvent event) {
    ...
    try {
      if (!message.getEmbeds().isEmpty()
          ...
          && Arrays.asList(reactions).contains(event.getEmoji().getAsReactionCode())
                  ...
                  .filter(
                      reaction ->
                          reaction
                              .getEmoji()
                              .getName()
                              .equals(Arrays.asList(reactions).get(finalIndex)))
