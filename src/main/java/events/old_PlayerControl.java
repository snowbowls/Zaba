...
public class old_PlayerControl extends ListenerAdapter {
  ...
  // play         - Plays songs from the current queue. Starts playing again if it was previously
  // paused
  ...
  // restart      - Restarts the current song or restarts the previous song if there is no current
  // song playing.
  ...
  public void onMessageReceived(MessageReceivedEvent event) {
    ...
    try {
      ...
      if (!allowedIds.contains(event.getAuthor().getId())) return;
    ...
    if ("!join".equals(command[0])) {
      if (connectedChannel == null) {
        ...
        try {
          ...
          if (e.getPermission() == Permission.VOICE_CONNECT) {
            event
                .getChannel()
                .sendMessage("Zaba does not have permission to connect to: " + chan.getName())
                .queue();
          }
        }
      }
      ...
      if (command.length == 1) // It is only the command to start playback (probably after pause)
      {
        ...
        // {
        //    event.getChannel().sendMessage("The current audio queue is empty! Add something to the
        // queue first!").queue();
        // }
      ...
      if (player.isPaused()) event.getChannel().sendMessage("The player has been paused.").queue();
      else event.getChannel().sendMessage("The player has resumed playing.").queue();
      ...
      if (command.length == 1) {
        event
            .getChannel()
            .sendMessage("Current player volume: **" + player.getVolume() + "**")
            .queue();
      } else {
        try {
          ...
          event
              .getChannel()
              .sendMessage("`" + command[1] + "` is not a valid integer. (10 - 100)")
              .queue();
        }
      }
      ...
      if (track == null)
        ...
        if (track != null) {
          ...
          event
              .getChannel()
              .sendMessage(
                  "No track has been previously started, so the player cannot replay a track!")
              .queue();
        }
    } else if ("!repeat".equals(command[0])) {
      // scheduler.setRepeating(!scheduler.isRepeating());
      // event.getChannel().sendMessage("Player was set to: **" + (scheduler.isRepeating() ?
      // "repeat" : "not repeat") + "**").queue();
