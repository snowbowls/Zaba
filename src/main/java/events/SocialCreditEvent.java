...
public class SocialCreditEvent extends ListenerAdapter {
  ...
  public void onMessageReceived(MessageReceivedEvent event) {
    ...
    String msgtemp = null;
    try {
      msgtemp = event.getMessage().getContentRaw();
        ...
        try (MongoClient mongoClient = MongoClients.create(settings)) {
          ...
          try {
            Bson projectionFields =
                Projections.fields(
                    Projections.include("username", "score"), Projections.excludeId());
            Document doc =
                collection.find(eq("userid", userid)).projection(projectionFields).first();
            if (doc == null) {
              event
                  .getChannel()
                  .sendMessage("idk who you are, go get some credit then come back")
                  .queue();
              System.err.println(
                  "DNE - "
                      + event.getMessage().getAuthor().getName()
        ...
        try (MongoClient mongoClient = MongoClients.create(settings)) {
          ...
          try {
            ...
            try (MongoCursor<Document> cursor =
              ...
              for (String s : currUsers) sbUser.append(s);
              for (String s : currCredit) sbCredit.append(s);
      ...
      // !show <user tag>
        ...
        userid =
            event
                .getMessage()
                .getContentRaw()
                .substring(9, event.getMessage().getContentRaw().length() - 1);
        try (MongoClient mongoClient = MongoClients.create(settings)) {
          MongoDatabase database = mongoClient.getDatabase("ChillGrill");
          MongoCollection<Document> collection = database.getCollection("socialcredit");
          try {
          ...
          try {
            Bson projectionFields =
                Projections.fields(
                    Projections.include("username", "score"), Projections.excludeId());
            Document doc =
                collection.find(eq("username", username)).projection(projectionFields).first();
            if (doc == null) {
              System.err.println("DNE");
            } else {
              event
    ...
    if (isCustom) {
      ...
      if (readId.equals("15_plus:900119408859578451") && !username.equals(reactor)) {
        try (MongoClient mongoClient = MongoClients.create(settings)) {
          ...
          Bson projectionFields =
              Projections.fields(
                  Projections.include("username", "score", "userid"), Projections.excludeId());
          Document doc = collection.find(eq("userid", userid)).projection(projectionFields).first();
          if (doc == null) {
        ...
        try (MongoClient mongoClient = MongoClients.create(settings)) {
          ...
          // Some weird function I wrote to annoy Stein, I'm unsure why the second condition exists
            ...
            if (Math.random() < .80)
              event
                  .getJDA()
                  .getUserById(event.getUser().getId())
                  .openPrivateChannel()
            ...
            Document doc1 =
                collection
                    .find(eq("userid", "251453431758192642"))
                    .projection(projectionFields)
                    .first();
            ...
            System.out.println(
                doc1.get("username")
                    ...
                    + currVal
                    + " New "
                    + doc1.getInteger("score")
                    + " --> they attacked the CCP");
            try {
    ...
    if (isCustom) {
      {
        ...
        if (event.getEmoji().getAsReactionCode().equals("15_plus:900119408859578451")
            && !username.equals(reactor)) {
          try (MongoClient mongoClient = MongoClients.create(settings)) {
            MongoDatabase database = mongoClient.getDatabase("ChillGrill");
            MongoCollection<Document> collection = database.getCollection("socialcredit");
            Bson projectionFields =
        ...
        // Trigger when message rem react -15
        if (event.getEmoji().getAsReactionCode().equals("15_neg:934919187787288597")
            && !username.equals(reactor)) {
          try (MongoClient mongoClient = MongoClients.create(settings)) {
            MongoDatabase database = mongoClient.getDatabase("ChillGrill");
            MongoCollection<Document> collection = database.getCollection("socialcredit");
